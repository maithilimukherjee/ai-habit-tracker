    ...
    <aside className="hidden md:flex md:flex-col w-64 fixed inset-y-0 left-0 z-30 glass border-r">
      ...
      <div className="p-3 border-t divider space-y-1">
        ...
        <div className="px-2 py-2 flex items-center gap-3">
          <div className="w-9 h-9 rounded-full bg-gradient-to-br from-brand-500 to-brand-700 text-white font-semibold flex items-center justify-center shadow-md shadow-brand-500/30 overflow-hidden flex-shrink-0">
            {user?.avatar ? (
              <img 
                src={user.avatar} 
                alt="Profile avatar" 
          ...
          <button
            onClick={logout}
            title="Log out"
            className="p-2 rounded-lg text-soft hover:bg-[var(--surface-hover)] flex-shrink-0"
          >
            <LogOut size={16} />
          </button>
        </div>
