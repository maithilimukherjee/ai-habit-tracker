...
export default function Habits() {
  ...
  const load = async () => {
    ...
    try {
      ...
      // FIX 1: Safely handle object or array format from logs range
      const logsData = rangeRes.data?.data || rangeRes.data || {};
      const logsArray = Array.isArray(logsData) ? logsData : Object.values(logsData).flat();
  ...
  const save = async (data) => {
    ...
    try {
      if (editing) {
        ...
        setLogsByHabit((p) => ({ ...p, [newHabit._id]: [] }));
      }
      setFormOpen(false);
      setEditing(null);
    } catch (err) {
      // FIX 2: Catching and printing error to console to diagnose payload rejections
      console.error("Error saving habit submission:", err?.response?.data || err);
    } finally {
      setSubmitting(false);
    }
  };
